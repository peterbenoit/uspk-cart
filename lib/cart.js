...
export const createBigCommerceCart = async (lineItems) => {
	try {
		const response = await client.post("/v3/carts", {
...
export const getBigCommerceCart = async (cartId) => {
	...
	try {
		const response = await client.get(`/v3/carts/${cartId}?include=line_items.physical_items.options,promotions.banners`);
...
export const addBigCommerceCartLineItems = async (cartId, lineItems) => {
	...
	try {
		const response = await client.post(`/v3/carts/${cartId}/items`, {
			line_items: formatLineItems(lineItems),
		});
		return response.data.data;
	} catch (error) {
 ...
 * @returns {Promise<Object>} The updated cart object from BigCommerce.
 */
export const updateBigCommerceCartItem = async (cartId, itemId, quantity) => {
	if (!cartId || !itemId) {
		console.error("updateBigCommerceCartItem called with no cartId or itemId.");
		throw new Error("Cannot update item without cart ID and item ID.");
	}
	try {
		const response = await client.put(`/v3/carts/${cartId}/items/${itemId}`, {
			line_item: { quantity: quantity }, // Note: BigCommerce expects quantity under line_item object
		});
		return response.data.data;
	} catch (error) {
		console.error(`Error updating item ${itemId} in BigCommerce cart ${cartId}:`, error.response?.data || error.message);
		throw error;
	}
};
...
export const deleteBigCommerceCartItem = async (cartId, itemId) => {
	...
	try {
		...
		const response = await client.delete(`/v3/carts/${cartId}/items/${itemId}?include=line_items`);
...
export const deleteBigCommerceCart = async (cartId) => {
	...
	try {
		await client.delete(`/v3/carts/${cartId}`);
	} catch (error) {
