...
export const getBigCommerceCart = async (cartId) => {
	...
	try {
		...
		if (error.response && (error.response.status === 404 || error.response.status === 403)) {
			return null; // Cart not found, likely expired or invalid, or forbidden
		}
		throw error;
	}
};
...
/**
 * Retrieves the checkout URL for a given cart.
 * @param {string} cartId - The ID of the cart.
 * @returns {Promise<string|null>} The checkout URL or null if not available.
 */
export const getBigCommerceCheckoutUrl = async (cartId) => {
	if (!cartId) {
		console.warn("getBigCommerceCheckoutUrl called with no cartId");
		return null;
	}
	try {
		const response = await client.post(`/carts/${cartId}/redirect_urls`);
		return response.data.data.checkout_url; // Or embedded_checkout_url depending on needs
	} catch (error) {
		console.error(`Error fetching checkout URL for cart ${cartId}:`, error.response?.data || error.message);
		// Handle specific errors, e.g., cart not found (404) or expired
		if (error.response?.status === 404) {
			// You might want to throw a specific error or return a specific value
			// to indicate the cart is not found or expired.
			console.warn(`Cart ${cartId} not found or expired when fetching checkout URL.`);
		}
